...
export const socialResolvers = {
  Query: {
    ...
    socialNetwork: async (_: any, { id }: { id: string }) => {
      try {
    ...
    socialNetworkBySlug: async (_: any, { slug }: { slug: string }) => {
      try {
        return await prisma.social.findUnique({
          where: { slug },
        });
  ...
  Mutation: {
    createSocial: async (_: any, data: any) => {
      try {
    ...
    updateSocial: async (_: any, { id, ...data }: any) => {
      try {
        // Verificar se existe
    ...
    deleteSocial: async (_: any, { id }: { id: string }) => {
      try {
        ...
        if (categoriesCount > 0) {
          throw new Error(
            'Não é possível excluir esta rede social pois existem categorias associadas a ela'
          );
        }
